module Botan
  def self.kdf(algo, secret, out_len, salt, label)
    ...
    rc = LibBotan.botan_kdf(algo, out_buf, out_buf.size, secret_buf, secret_buf.size, salt_buf, salt_buf.size, label_buf, label_buf.size)
    raise if rc != 0
  ...
  def self.pbkdf(algo, password, out_len, iterations=10000, salt=RNG.new.get(12))
    ...
    salt_buf = FFI::MemoryPointer.new(:uint8, salt.bytesize)
    salt_buf.write_bytes(salt)
    rc = LibBotan.botan_pbkdf(algo, out_buf, out_len, password, salt_buf, salt_buf.size, iterations)
    raise if rc != 0
  ...
  def self.pbkdf_timed(algo, password, out_len, ms_to_run=300, salt=RNG.new.get(12))
    ...
    rc = LibBotan.botan_pbkdf_timed(algo, out_buf, out_len, password, salt_buf, salt_buf.size, ms_to_run, iterations_ptr)
    raise if rc != 0
